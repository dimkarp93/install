...
Имя исполняемого файла берётся из файла `bin.txt` (если он есть), иначе — из имени каталога
репозитория. Анализировать файл workflow больше не нужно.
...
- **Имя исполняемого файла**: совпадает с именем каталога либо задано в `bin.txt` (одна
  строка).

## Проверка соответствия конвенциям (`check.sh`)

`check.sh` проверяет, что репозиторий соответствует [CONVENTIONS.md](CONVENTIONS.md):
наличие `.git`, корректный `versions.txt`, имя исполняемого файла (`bin.txt` или имя
каталога), цель сборки, цели `bump-*`, workflow релиза и формат git-тегов. Аргументы — те
...
3. **Иметь имя исполняемого файла**, равное имени репозитория либо заданное в `bin.txt`.
...
- определяет имя исполняемого файла из `bin.txt` (или имени репозитория),
