...
check:
    ...
    for pair in workflow-go-github:release.yml workflow-go-gitea:release-gitea.yml \
                workflow-sh-github:release-sh.yml workflow-sh-gitea:release-sh-gitea.yml; do
        t=${pair%%:*}
        f=${pair#*:}
...
smoke:
    ...
    tmp=$(mktemp -d)
    trap 'rm -rf "$tmp"' EXIT
    ./init_install.sh --lang go --owner dimkarp93 --ci both --no-git "$tmp/smokego" >/dev/null
    ./init_install.sh --lang sh --no-git "$tmp/smokesh" >/dev/null
